...
static bool load_shader(const char* path, ShaderCode& code) noexcept
{
    ...
    if (!file)
    {
        fprintf(stderr, "Cannot open task test shader: %s\n", path);
        return false;
    }
...
static bool initialize(Fixture& fixture) noexcept
{
    ...
    if (!load_shader(NOGRAPHICSAPI_TASK_TEST_TASK_SPV, task) || !load_shader(NOGRAPHICSAPI_TASK_TEST_MESH_SPV, mesh) ||
        !load_shader(NOGRAPHICSAPI_TASK_TEST_FRAGMENT_SPV, fragment))
        return false;
    fixture.pso = gpu::create_mesh_pso(
        fixture.device,
        {
...
static bool render_case(Fixture& fixture, uint32 count, uint32 visible_mask, uint32 scenario, bool indirect) noexcept
{
    ...
    for (uint32 id = 0; id < task_test_capacity; ++id)
        seed.visibility[id] = 1u << (id % 3);
    for (uint32 batch = 0; batch < task_test_batches; ++batch)
        seed.batch_counts[batch] = ~0u;
    ...
    *reinterpret_cast<gpu::uint32x3*>(fixture.seed.range.cpu + sizeof(TaskTestData)) = { .x = batches, .y = 1, .z = 1 };
    ...
    gpu::barrier(
        commands,
        gpu::Stage::task | gpu::Stage::mesh | gpu::Stage::transfer,
        gpu::Access::shader_write | gpu::Access::transfer_read,
        gpu::Stage::transfer,
    ...
    gpu::barrier(
        ...
        gpu::Stage::transfer,
        gpu::Access::transfer_write,
        gpu::Stage::task | gpu::Stage::mesh | gpu::Stage::indirect,
    ...
    gpu::copy_memory(
        commands,
        { .gpu = fixture.data.range.gpu, .size = sizeof(TaskTestData) },
    ...
    for (uint64 offset = sizeof(TaskTestData); offset < pixel_offset; ++offset)
        valid &= fixture.readback.range.cpu[offset] == 0xa5;
    ...
    for (uint32 y = 0; y < task_test_height; ++y)
        for (uint32 x = 0; x < task_test_width; ++x)
        {
            ...
            valid &= pixel[0] == (covered ? 255 : 0) && pixel[1] == (covered && (scenario & 1u) ? 255 : 0) && pixel[2] == (covered && indirect ? 255 : 0) &&
                     pixel[3] == 255;
        }
    if (!valid)
        fprintf(stderr, "Task shader %s case %u failed: count=%u mask=%u.\n", indirect ? "indirect" : "direct", scenario, count, visible_mask);
    return valid;
}
