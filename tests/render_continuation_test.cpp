...
namespace {
    ...
    bool load_shader(const char* path, ShaderCode& code) noexcept
    {
        ...
        if (!file)
        {
            fprintf(stderr, "Cannot open render continuation shader: %s\n", path);
            return false;
        }
        fseek(file, 0, SEEK_END);
        const long bytes = ftell(file);
        rewind(file);
        if (bytes < 20 || uint64(bytes) > sizeof(code.words) || (bytes & 3))
        {
            fclose(file);
            return false;
        }
        ...
        fclose(file);
        if (!read || (code.words[0] != 0x07230203u && code.words[0] != 0x43425844u))
            return false;
    ...
    void draw_segment(gpu::CommandBuffer* commands, const Worker& worker) noexcept
    {
        ...
        if (worker.index != 0)
            flags = flags | gpu::RenderingFlags::resuming;
        if (worker.index != segment_count - 1)
            flags = flags | gpu::RenderingFlags::suspending;
        gpu::begin_render_pass(commands, *worker.rendering, flags);
        if ((worker.iteration & 1u) == 0)
            gpu::write_timestamp(commands, worker.timestamps);
        ...
        gpu::draw(commands, {}, 3);
        if ((worker.iteration & 1u) == 0)
            gpu::write_timestamp(commands, worker.timestamps + 1);
    ...
    bool record_workers(gpu::Span<Worker> workers) noexcept
    {
        ...
        for (; started < workers.size; ++started)
        {
            ...
            if (!workers.data[started].thread)
                break;
#else
            if (pthread_create(&workers.data[started].thread, nullptr, record_segment, workers.data + started) != 0)
                break;
#endif
        }
#if defined(_WIN32)
...
#endif
        if (!valid)
            fprintf(stderr, "Render continuation worker creation or join failed.\n");
        return valid;
    }
    ...
    bool initialize(Fixture& fixture) noexcept
    {
        ShaderCode vertex{};
        ShaderCode fragment{};
        if (!load_shader(NOGRAPHICSAPI_CONTINUATION_VERTEX_SPV, vertex) || !load_shader(NOGRAPHICSAPI_CONTINUATION_FRAGMENT_SPV, fragment))
            return false;
        fixture.pso = gpu::create_graphics_pso(
            fixture.device,
            {
    ...
    bool render_iteration(Fixture& fixture, uint32 iteration) noexcept
    {
        ...
        if (iteration < 4)
        {
            if (!record_workers(fixture.workers))
                return false;
        }
        else
        {
        ...
        {
            if (iteration & 1u)
                valid &= timestamps[index] == ~uint64{ 0 };
            else
                valid &= timestamps[index] != ~uint64{ 0 } && (index == 0 || timestamps[index] >= timestamps[index - 1]);
        }
        if ((iteration & 1u) == 0)
        {
        ...
        if (!valid)
            fprintf(stderr, "Render continuation pixel or timestamp readback failed in iteration %u.\n", iteration);
        return valid;
    }
...
int main()
{
    ...
    if (initialized.error == gpu::Error::unsupported)
        return 77;
    if (initialized.error != gpu::Error::none)
        return 1;
