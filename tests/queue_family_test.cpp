...
static bool initialize(Fixture& fixture) noexcept
{
    ...
    if (!file)
    {
        fprintf(stderr, "Cannot open queue-family test shader.\n");
        return false;
    }
    uint32 code[8192]{};
    const size_t size = fread(code, 1, sizeof(code), file);
    const bool valid = size >= 20 && size % sizeof(uint32) == 0 && !ferror(file) && feof(file) && (code[0] == 0x07230203u || code[0] == 0x43425844u);
    fclose(file);
    if (!valid)
    {
        fprintf(stderr, "Invalid queue-family test shader.\n");
        return false;
    }
    ...
    for (uint32 index = 0; index < 3; ++index)
        fixture.pools[index] = create_command_pool(fixture.device, fixture.queues[index]);
    for (TimelineSemaphore*& signal : fixture.signals)
        signal = create_timeline_semaphore(fixture.device);
    ...
    const TextureDesc description{
        ...
        .usage =
            TextureUsage::sampled | TextureUsage::storage | TextureUsage::color_attachment | TextureUsage::transfer_source | TextureUsage::transfer_destination,
    };
    ...
    write_texture_descriptor(fixture.device, fixture.descriptors.range.cpu + caps.texture_descriptor_size, fixture.textures[1], TextureDescriptorType::storage);
...
{
    ++fixture.value;
    for (CommandPool* pool : fixture.pools)
    ...
    if (copy_timestamps)
        write_timestamp(producer, timestamp_gpu);
    ...
    if (copy_timestamps)
        write_timestamp(producer, timestamp_gpu + 1);
    ...
    if (copy_timestamps)
        write_timestamp(readback, timestamp_gpu + 6);
    ...
    if (copy_timestamps)
        write_timestamp(readback, timestamp_gpu + 7);
    ...
    const SubmitDesc produced{ .commands = { commands, 1 }, .waits = { waits, 1 }, .completion = { .semaphore = fixture.signals[0], .value = fixture.value } };
    const SubmitDesc computed{ .commands = { commands + 1, 1 },
                               .waits = { waits + 1, 1 },
                               .completion = { .semaphore = fixture.signals[1], .value = fixture.value } };
    const SubmitDesc rendered{ .commands = { commands + 2, 1 },
                               .waits = { waits + 2, 1 },
    ...
    submit(
        fixture.device,
        { .commands = { readback },
          .waits = { { .semaphore = fixture.signals[2], .value = fixture.value } },
          .completion = { .semaphore = fixture.signals[3], .value = fixture.value } },
        fixture.queues[0]
    );
    ...
    {
        if (!copy_timestamps && (index == 0 || index == 6))
            continue;
        valid =
            timestamps[index] != 0xa5a5a5a5a5a5a5a5ull && timestamps[index + 1] != 0xa5a5a5a5a5a5a5a5ull && timestamps[index + 1] >= timestamps[index] && valid;
    }
    for (uint64 index = 0; index < fixture.readback.range.size; ++index)
    {
        if (index < timestamp_offset ? index % slot_bytes < data_bytes : index < timestamp_offset + 8 * sizeof(uint64))
            continue;
        valid = fixture.readback.range.cpu[index] == 0xa5 && valid;
    }
    if (!valid)
        fprintf(stderr, "Queue-family data, texture, timestamp, or guard check failed at iteration %u.\n", iteration);
    return valid;
}
...
static void shutdown(Fixture& fixture) noexcept
{
    ...
    for (TimelineSemaphore* signal : fixture.signals)
        destroy_timeline_semaphore(signal);
    destroy_render_view(fixture.target);
    for (Texture* texture : fixture.textures)
...
int main(int argc, char** argv)
{
    ...
    const DeviceInit initialized =
        create_device({ .desired_compute_queue_count = compute ? 32u : 0u, .desired_copy_queue_count = copy ? 32u : 0u, .timestamp_query_count = 2 });
    ...
    const DeviceCaps& caps = get_device_caps(fixture.device);
    printf(
        "%s: %u general, %u compute, %u copy queues; copy granularity %u x %u x %u.\n",
        caps.device_name,
        caps.general_queue_count,
        caps.compute_queue_count,
        caps.copy_queue_count,
        caps.copy_texture_granularity.x,
        caps.copy_texture_granularity.y,
        caps.copy_texture_granularity.z
    );
    if (copy && !copy_timestamps)
        printf("Copy-queue timestamps omitted: see docs/known-driver-issues.md and --copy-timestamps.\n");
    ...
    if (valid)
        for (uint32 iteration = 1; iteration <= 8; ++iteration)
            valid = run_case(fixture, iteration) && valid;
    shutdown(fixture);
    return valid ? 0 : 1;
}
